...
def test_preferences_dialog_autofills_diameters_from_face(qtbot):
    """Picking a different face in the dropdown populates the calibre and
...
def test_preferences_dialog_inserts_no_camera_when_saved_is_missing(qtbot):
    """If the saved camera isn't in the enumerated list, the dropdown
...
def test_preferences_dialog_emits_camera_changed_on_user_pick(qtbot):
    """Selecting a different camera sends ``camera_changed`` with the
...
def test_preferences_dialog_set_camera_options_replaces_dropdown(qtbot):
    """``set_camera_options`` rebuilds the dropdown and keeps the user's
...
def test_preferences_dialog_set_camera_options_falls_back_to_no_camera(qtbot):
    ...
    list, the dropdown falls back to "No camera"."""
    from shottrainer.ui.preferences_dialog import Preferences, PreferencesDialog
