...
Kept separate from the user's preferences because window
placement is per-machine and changes a lot more often than
anything in preferences. The file is small, easy to reason about,
and the app survives without it.
"""
...
def decode_geometry(encoded: str) -> bytes:
    ...
    Returning empty bytes means
    ``QMainWindow.restoreGeometry`` simply does nothing rather
    than raising, so a corrupt file just falls back to the
    default sizing.
    """
    if not encoded:
        return b""
