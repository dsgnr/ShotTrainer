...
and sends out a signal when it spots one. Handy when an external
...
class SettingsWatcher(QObject):
    """Sends out ``changed(Preferences)`` when ``settings.json`` is updated."""
    ...
    def mark_seen(self) -> None:
        """Reset the baseline so an external write triggers it but our own doesn't."""
        self._mtime = self._current_mtime()
