...
Small utilities used by multiple UI modules. Keeps duplicated creation
logic in one place so style and behaviour changes only need to happen
once.
"""
...
def make_combo(
    ...
    *,
    ...
    Args:
        items: List of (data, display_label) pairs for the combo entries.
...
class PropertySlider(QWidget):
    ...
    to ``default`` and fires the change so the live preview updates.
    """
